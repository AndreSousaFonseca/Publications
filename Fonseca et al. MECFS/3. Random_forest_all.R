#Random Forest (HCs vs. all EBV individuals)
...
# Add a varaible identifying healthy and CFS patietns
...
#
#saveRDS(rf_info_data, "rf_info_data_both.RDS") # Save the results form Random Froest
rf_info_data <- readRDS("rf_info_data_both.RDS") # Load the results from Random Forest
...
#
##Save the data
#saveRDS(mean_imp , "rf.mean_both.RDS")
mean_imp <- readRDS("rf.mean_both.RDS")
...
sup2 <- ggplot(mean_imp, aes(x = Antibody , y=Importance_Mean, color = as.factor(Protein))) +
  ...
  ggtitle("HCs against all ME/CFS patients") +
  theme(plot.title = element_text(hjust = 0.5))

saveRDS(sup2, "sup2(A).RDS")


sup1 <-  ggplot(mean_imp, aes(x = Protein , y=log10(Importance_Mean), fill = as.factor(Protein))) +
  geom_boxplot()+theme_bw() +
  ylim(c(-3,0)) +
  theme_bw() + ylab("log10(Mean importance)") +
...
#1500 X 700
 ...
 #   geom_boxplot()+theme_bw() +
 #   ylim(c(0.,0.4)) +
 #   theme_bw() + ylab("Mean antibody mportance") +
...
ggplot(mean_prot, aes(x = Protein , y=Importance_Mean, color = as.factor(Protein), fill = as.factor(Protein))) + geom_bar(stat="identity") +theme_bw()
...
dt
write.csv(dt$res,"table_portein-wise_both.csv")
...
#plb <- ggplot(best.results, aes(x = reorder(Antibody, pos), y = Importance_Mean, fill = Antibody)) +  
#  geom_bar(stat = "identity" ) + coord_flip() + guides(fill="none") + ylim(c(0, 0.4)) + 
#  theme_classic()  + xlab("Antibody") + ylab("Mean Importance")
#plb 

#best.results$col <-  rev(viridis::viridis(n=10))
#plb <- ggplot(best.results, aes(x = reorder(Antibody, pos), y = Importance_Mean, fill = as.factor(pos))) +  
#  geom_bar(stat = "identity" ) + coord_flip() + guides(fill="none") + ylim(c(0, 0.4)) + 
#  theme_classic()  + xlab("Antibody") + ylab("Mean Importance")
#plb + scale_fill_manual(values = best.results$col)

best.results$col <-  rev(viridis::plasma(n=10))
...
plg <- ggplot(ab_box, aes(x=reorder(Antibody, Importance), Importance, fill = reorder(Antibody, Importance))) +
  ...
  scale_fill_manual(values = best.results$co) + theme_classic()  + 
...
# Position plot
mean_imp$location <- substr(mean_imp$Antibody, 11, 20)
ll <-  as.numeric(stringr::str_extract(mean_imp$location, "[^_]+"))


ggplot(mean_imp, aes(Antibody, Importance_Mean, color = as.factor(Protein))) +
  geom_point() +
  scale_fill_manual(values =col) +
  theme(panel.grid.major = element_blank(),
        panel.grid.minor = element_blank()) + 
  facet_wrap(~ as.factor(Protein), scales = "free_x") +
  scale_x_discrete(labels=c(ll))
...
saveRDS(slearn.subsets, "slearn.subsets.both.RDS")
slearn.subsets <- readRDS("slearn.subsets.both.RDS")


# Perform a model with the single topmost antibody:
#logistic <- glm(y_train ~ train_set$EBV_EBNA3_0139, family = binomial(link="logit"))
#logistic_pred <- ifelse(logistic$fitted.values > 0.5, 1, 0)
#confusionMatrix(as.factor(y_train), as.factor(logistic_pred))
#rf <- randomForest::randomForest(as.factor(y_train) ~ train_set$EBV_EBNA3_0139)
#confusionMatrix(as.factor(y_train), rf$predicted)
...
plt2 <- ggplot(acc.grahps, aes(x= Number_antibody, y= ROC01, color = Subset ))+ geom_line(size=1) +
  ...
  theme(panel.grid.major = element_blank(),
        panel.grid.minor = element_blank()) +
  geom_point(aes(x=39, y=0.64), colour="grey9") +
  annotate("text", x=39 , y= 0.67, label= "Best classifier",  colour="grey9") +
...
title <- ggdraw() + 
  draw_label("HCs against all ME/CFS patients",
             x=0.40,
             hjust = 0, size =15
  ) +
  theme(
...
fig1
saveRDS(fig1, "fig1(A).RDS")
#1100 * 1300


accuracies_testing$id <-  seq(1:nrow(accuracies_testing))
slearn.subsets[[50]]$coef

accuracies_testing$id <-  seq(1:nrow(accuracies_testing))
write.csv(slearn.subsets[[50]]$varNames, "no_antibodies.csv")

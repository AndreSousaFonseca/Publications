#Random Forest (HCs vs. EBV infect individuals)
...
# Subset the 50 healthy and 54 infected
data <- subset(dataset, dataset$Status == "healthy" | dataset$Infection == "ja" )
dim(data)
...
#
#saveRDS(rf_info_data, "rf_info_data_yes.RDS") # Save the results form Random Froest
rf_info_data <- readRDS("rf_info_data_yes.RDS") # Load the results from Random Forest
...
# Save the data
#saveRDS(mean_imp , "rf.mean_yes.RDS")
...
ggtitle("HCs against ME/CFS with putative infection") +
  theme(plot.title = element_text(hjust = 0.5))

saveRDS(sup2, "sup2(C).RDS")

sup1 <- ggplot(mean_imp, aes(x = Protein , y=Importance_Mean, fill = as.factor(Protein))) +
  geom_boxplot()+theme_bw() +
  ylim(c(0.,0.4)) +
  theme_bw() + ylab("Mean mportance") +
...
#plt1 <- plt + annotation_custom(ggplotGrob(plg),
#                        xmin = 0.15, ymin = 10,
#                        xmax = 0.3, ymax = 90)
#
...
#}
#saveRDS(slearn.subsets, "slearn.subsets.RDS")
slearn.subsets <- readRDS("slearn.subsets.RDS")
...
sup3 <- ggplot(models_training, aes(x =Number_antibody, y =Values , color = Method)) + geom_line(size=1) + theme_bw() +
  theme(panel.grid.major = element_blank(),
        panel.grid.minor = element_blank()) + xlab("Number of antibodies") + ylab("Accuracy") +
...
#1200 X  700
sup3
saveRDS(sup3, "sup3(C).RDS")


coefficient_table <-  data.frame()
for (model in slearn.subsets) {
  coef <- data.frame( model$coef, length(model$varNames))
  coefficient_table <- rbind(coefficient_table, coef)
}

coefficient_table$Model <- as.factor(gsub("_.*","",rownames(coefficient_table)))
names(coefficient_table)[1:2] <-  c("Coefficient", "Number_antibody")

# Plot the coefficients
sup4 <- ggplot(coefficient_table, aes(x =Number_antibody, y =Coefficient , color = Model)) + geom_line(size=1) + geom_point() + theme_bw() +
  theme(panel.grid.major = element_blank(),
        panel.grid.minor = element_blank()) + xlab("Number of antibodies") + ylab("Coefficient") +
  scale_color_manual(values = c("#FD636B", "#ECB22E", "#CCABDB", "#2EB67D", "#36C5F0")) + labs(color='Model') +
  ggtitle("HCs against ME/CFS with putative infection") +
  theme(plot.title = element_text(hjust = 0.5))

sup4

slearn.subsets[[43]]$coef
...
plt2 <- ggplot(acc.grahps, aes(x= Number_antibody, y= ROC01, color = Subset ))+ geom_line(size=1) +
  ...
  theme(panel.grid.major = element_blank(),
        panel.grid.minor = element_blank()) +
  geom_point(aes(x=27, y=0.9), colour="grey9") +
  annotate("text", x=27 , y= 0.93, label= "Optimal classifier",  colour="grey9") +
...
title <- ggdraw() + 
  draw_label("HCs against ME/CFS with putative infection",
             x=0.36,
             hjust = 0, size =15
  ) +
  theme(
...
write.csv(slearn.subsets[[43]]$varNames, "yes_antibodies.csv")
read.csv("yes_antibodies.csv")


#Obtain the pred values 
X <- data.frame("X"= slearn.subsets[[43]]$SL.predict[,1], "Y" = y_train )
opt.spse <- optimal.cutpoints("X", "Y", tag.healthy = 0, method = "SpEqualSe", data = X)
s.sesp <- summary(opt.spse)
sesp <-  s.sesp$p.table$Global$SpEqualSe[[1]][1]
predicted <- predict(slearn.subsets[[43]], test_subsets[[43]], onlySL = TRUE)
pred <-  ifelse(predicted$pred > sesp, 1 ,0)
cf.sesp <- confusionMatrix(as.factor(pred), as.factor(y_test), positive = "1")
cf.sesp

#fourfoldplot(cf.sesp$table, color= c("#F7A4A4", "#FF6464"), conf.level = 0, std = "all.max")
fourfoldplot(matrix(c(5,0,1,4), nrow = 2 ), color= c("#FEB92CFF", "#F79044FF"), conf.level = 0, std = "all.max")


roc <- roc(as.factor(y_train) , slearn.subsets[[43]]$SL.predict[,1])
plot.roc(roc, col= "#9512A1FF", legacy.axes = T)

predicted <- predict(slearn.subsets[[43]], test_subsets[[43]], onlySL = TRUE)
roc <- roc(as.factor(y_test) , predicted$pred[,1])
plot(roc, col= "#F79044FF", add=T, legacy.axes = T, )



#slearn.subsets[[43]]$varNames[order((slearn.subsets[[43]]$varNames))]
unique_prots <- stringr::str_extract(slearn.subsets[[43]]$varNames, "[^_]*_[^_]*")
unique(unique_prots)
table(unique_prots)
unique(mean_imp$Protein)


# Obtain the sequence for the 27 antibodies
inf_seq <- seq_data[colnames(seq_data) %in% slearn.subsets[[43]]$varNames]
info_seq <-  data.frame("Antibody" = colnames(inf_seq), "Sequence"  = t(as.vector(inf_seq[2,])))


most_imp <-  mean_imp[ mean_imp$Antibody %in% info_seq$Antibody, ]
